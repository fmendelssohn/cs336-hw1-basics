...
from cs336_basics.utils.nn_utils import *
...
def run_multihead_self_attention_with_rope(
    ...
    """
    d_k = d_v = d_model // num_heads
    positional_encoder = RotaryPositionalEmbedding(theta, d_model // num_heads, max_seq_len, device=in_features.device)
    model = CausalMultiheadSelfAttentionWithRope(d_model, num_heads, positional_encoder=positional_encoder)
    model.load_state_dict({
        "q_proj.weight": q_proj_weight,
        "k_proj.weight": k_proj_weight,
        "v_proj.weight": v_proj_weight,
...
def run_rope(
    ...
    """
    rope = RotaryPositionalEmbedding(theta=theta, dim=d_k, context_length=max_seq_len)
...
def run_transformer_block(
    ...
    """
    d_k = d_v = d_model / num_heads
    rope = RotaryPositionalEmbedding(theta=theta, dim=d_k, context_length=max_seq_len, device=in_features.device)
    model = TransformerBlock(d_model, num_heads=num_heads, d_ff=d_ff, positional_encoder=rope, device=in_features.device)
...
def run_gradient_clipping(parameters: Iterable[torch.nn.Parameter], max_l2_norm: float) -> None:
    ...
    """
    clip_gradients(parameters, max_l2_norm=max_l2_norm)
    return
